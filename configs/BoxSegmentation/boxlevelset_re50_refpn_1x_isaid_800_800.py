...
model = dict(
    ...
    boxseg_head=dict(
        type='BoxLevelsetHead',
        fc_out_channels=1024,
        featmap_strides=[4, 8, 16, 32],
        num_classes=16,
        loss_cls=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0),
        loss_bbox_init=dict(type='ChamferLoss2D', use_cuda=True, loss_weight=0.5, eps=1e-8),
        loss_bbox_refine=dict(type='ChamferLoss2D', use_cuda=True, loss_weight=1.0, eps=1e-8),
...
data_root = '/data/iSAID_patches_800/'
img_norm_cfg = dict(
...
data = dict(
    ...
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'val/instances_val.json',
...
log_level = 'INFO'
work_dir = 'work_dirs/boxlevelset_re50_refpn_1x_isaid'
load_from = None
resume_from = None
workflow = [('train', 1)]
