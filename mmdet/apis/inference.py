...
def draw_poly_mask_detections(img, detections_tuple, class_names, scale, threshold=0.2, putText=True, showStart=False, colormap=None):
    """
    :param img:
    :param detections:
    :param class_names:
    ...
    class_names = ['Small_Vehicle', 'storage_tank', 'Large_Vehicle', 'plane', 'ship', 'Swimming_pool', 'Harbor',
                    'tennis_court', 'Ground_Track_Field', 'Soccer_ball_field', 'baseball_diamond', 'Bridge', 'basketball_court',
    ...
    for j, name in enumerate(class_names):
        ...
        if len(dets) > 0:
            ...
            for i, det in enumerate(dets):
                ...
                color = tuple(color_mask.tolist()[0])
                if showStart:
                    cv2.circle(img, (bbox[0], bbox[1]), 3, (0, 0, 255), -1)
                for i in range(3):
                    cv2.line(img, (bbox[i * 2], bbox[i * 2 + 1]), (bbox[(i+1) * 2], bbox[(i+1) * 2 + 1]), color=color, thickness=1, lineType=cv2.LINE_AA)
                cv2.line(img, (bbox[6], bbox[7]), (bbox[0], bbox[1]), color=color, thickness=1, lineType=cv2.LINE_AA)
                if putText:
                    cv2.putText(img, '%s %2d' % (class_names[j], score * 100.0) + '%', (bbox[6], bbox[7]),
                                color=color, fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.35)
