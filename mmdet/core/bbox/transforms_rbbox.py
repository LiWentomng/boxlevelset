...
def best_match_dbbox2delta(Rrois, gt, means = [0, 0, 0, 0, 0], stds=[1, 1, 1, 1, 1]):
    ...
    # print('gt_boxes_new', gt_boxes_new.shape, gt_boxes_new)
    try:
...
def best_match_rbox(Rrois, gt):
    ...
    #     pdb.set_trace()
    # bbox_targets = dbbox2delta_v2(Rrois, gt_boxes_new, means, stds)
    bbox_targets = gt_boxes_new
    # print('bbox_targets', bbox_targets.shape, bbox_targets)
...
def gt_mask_bp_obbs_list(gt_masks_list):
    # print('gt_masks_list1', gt_masks_list)
    gt_obbs_list = map(gt_mask_bp_obbs, gt_masks_list) # map(f, list)  return new_list

    return list(gt_obbs_list)


def gt_mask_bp_obbs(gt_masks, with_module=True):
    # print('gt_masks2', gt_masks)
...
def down_mask_list(mask_list):

    gt_mask_down_list = map(down_mask, mask_list)
    return list(gt_mask_down_list)

def down_mask(gt_masks):

    gt_mask_down = map(down_mask_single, gt_masks)
    gt_mask_down_np = np.stack(list(gt_mask_down), axis=0)
    return gt_mask_down_np

def down_mask_single(gt_masks):
    stride = 4
    start = int(stride // 2)
    gt_masks_down = gt_masks[start::stride, start::stride]
    return gt_masks_down
...
def polybox2result(bboxes, labels, num_classes):
    """Convert detection results to a list of numpy arrays.
    ...
    if bboxes.shape[0] == 0:
        ...
        tmp_bboxes = get_horizontal_bboxes(bboxes[:, :-1])
        bboxes = torch.cat([tmp_bboxes, bboxes[:, -1:]], dim=1)
        # import pdb
        # pdb.set_trace()
